...
function AdminDashboard({ onLogout }) {
  ...
  const updateMissionAndCalculate = (updates) => {
    ...
    const montant = calculateMontant(
      updated.duree_heures,
      updated.duree_minutes,
      updated.tarif_horaire,
    );
  ...
  const getFilteredMissions = () => {
    return missions.filter((mission) => {
      ...
      if (missionFilters.dateFrom) {
        if (new Date(mission.date_mission) < new Date(missionFilters.dateFrom))
          return false;
      }

      // Filtre par date (au)
      if (missionFilters.dateTo) {
        if (new Date(mission.date_mission) > new Date(missionFilters.dateTo))
          return false;
      }

      // Filtre par service
      if (missionFilters.service && mission.service !== missionFilters.service)
        return false;

      // Filtre par intervenant
      if (
        missionFilters.intervenant &&
        mission.intervenant_id !== missionFilters.intervenant
      )
        return false;

      // Filtre par client
      if (missionFilters.client && mission.client_id !== missionFilters.client)
        return false;
      // Filtre par statut
  ...
  const deleteIntervenant = async (intervenantId) => {
    if (
      window.confirm("Êtes-vous sûr de vouloir supprimer cette intervenante ?")
    ) {
      try {
        const { error } = await supabase
          .from("intervenants")
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          {/* MENU */}
          <div
            className={`${mobileMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row gap-2 md:gap-0`}
          >
            {/* Accueil */}
            <button
              onClick={() => {
                  ...
                  >
                    ...
                    {[...Array(12)].map((_, i) => (
                      <option key={i + 1} value={i + 1}>
                        {new Date(2024, i).toLocaleString("fr-FR", {
                          month: "long",
                        })}
                      </option>
                    ))}
              ...
              {loadingMissions ? (
                ...
                <div className="space-y-4">
                  {filteredMissions.map((mission) => (
                    <div
                      key={mission.id}
                      className="border-2 border-gray-300 rounded-lg p-6 hover:shadow-lg transition"
                    >
                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              >
                                ...
                                <option value="Repassage">👕 Repassage</option>
                                <option value="Courses">🛒 Courses</option>
                                <option value="Diététique">
                                  🥗 Diététique
                                </option>
                                <option value="Aide au repas">
                                  🍽️ Aide au repas
                                </option>
                              </select>
                            </div>
                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="font-bold text-lg">
                                {getClientName(mission.client_id)}
                              </p>
                              ...
                              <p className="font-bold">
                                {getIntervenantName(mission.intervenant_id)}
                              </p>
                            ...
                            <div className="flex flex-col justify-between">
                              <div>
                                ...
                                <div
                                  className={`inline-block px-4 py-2 rounded-lg font-semibold ${getStatusColor(mission.status)}`}
                                >
                              ...
                              <div className="flex flex-wrap gap-2 mt-4">
                                ...
                                <button
                                  onClick={() =>
                                    updateMissionStatus(mission.id, "confirmée")
                                  }
                                  className={`px-3 py-1 rounded text-sm font-medium transition ${
                                ...
                                <button
                                  onClick={() =>
                                    updateMissionStatus(mission.id, "annulée")
                                  }
                                  className={`px-3 py-1 rounded text-sm font-medium transition ${
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData,
                                  anciennete_annees: e.target.value,
                                })
                              }
                              className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg"
                            />
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData,
                                  secteur_intervention: e.target.value,
                                })
                              }
                              className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg"
                            >
                            ...
                            <label className="flex items-center gap-2 mt-6">
                              <input
                                ...
                                onChange={(e) =>
                                  setEditFormData({
                                    ...editFormData,
                                    accepte_charte: e.target.checked,
                                  })
                                }
                      ...
                      <>
                        ...
                        {intervenant.anciennete_annees && (
                          <p className="text-gray-600">
                            📅 {intervenant.anciennete_annees} ans d'expérience
                          </p>
                        )}
                        {intervenant.secteur_intervention && (
                          <p className="text-gray-600">
                            🗺️ Secteur: {intervenant.secteur_intervention}
                          </p>
                        )}

                        {intervenant.services &&
                          intervenant.services.length > 0 && (
                            <p className="text-gray-600">
                              🛎️ Services: {intervenant.services.join(", ")}
                            </p>
                          )}
                    ...
                    {editingClient === client.id ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <select
                            value={editFormData.type_client || "particulier"}
                            onChange={(e) =>
                              setEditFormData({
                                ...editFormData,
                        ...
                        <div>
                          <label className="block text-gray-700 font-bold mb-2">
                            Souhaits / Préférences (max 200 caractères)
                          </label>
                          ...
                          <p className="text-sm text-gray-500 mt-1">
                            {(editFormData.souhaits || "").length}/200
                          </p>
                        </div>
                        <div className="flex gap-2">
                          <button
                      ...
                      <>
                        ...
                        {client.souhaits && (
                          <p className="text-gray-600 mt-2">
                            <strong>Souhaits:</strong> {client.souhaits}
                          </p>
                        )}
                        <div className="flex gap-2 mt-4">
                          <button
