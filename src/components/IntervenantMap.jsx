    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-8 rounded-lg shadow-lg">
        ...
        <p className="text-blue-100">
          Visualisez les clients et optimisez les trajets
        </p>
      </div>
        ...
        >
          ...
          {intervenants.map((intervenant) => (
            <option key={intervenant.id} value={intervenant.id}>
              {intervenant.name} (
              {intervenant.secteur_intervention || "Non défini"})
            </option>
          ))}
        </select>
      </div>
                ...
                {missions.map((mission) => (
                  <tr key={mission.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-center">
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold ${getStatusColor(mission.status)}`}
                      >
                        {mission.status}
                      </span>
                    </td>
          ...
          <div className="mt-6 grid md:grid-cols-3 gap-4">
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              <p className="text-green-600 text-sm font-bold">
                Distance moy (HQ→Client)
              </p>
              <p className="text-2xl font-bold text-green-700">
                {(
                  missions.reduce(
                    (sum, m) => sum + parseFloat(m.distance_pantoufles || 0),
                    0,
                  ) / missions.length
                ).toFixed(2)}{" "}
                km
            ...
            <div className="bg-purple-50 p-4 rounded-lg">
              <p className="text-purple-600 text-sm font-bold">
                Distance moy (Intervenante→Client)
              </p>
              <p className="text-2xl font-bold text-purple-700">
                {(
                  missions.reduce(
                    (sum, m) => sum + parseFloat(m.distance_intervenant || 0),
                    0,
                  ) / missions.length
                ).toFixed(2)}{" "}
                km
